...
function TeamBadge({ code, team }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-lg border border-black/10 dark:border-white/10 bg-white/60 dark:bg-white/5 px-2 py-1">
      <img src={`/teams/${team.slug}.png`} alt={team.name} className="h-7 w-7 rounded object-cover" />
...
export default function FullSchedule({ locale='nl' }) {
  ...
  function MatchCell({ match, event }) {
    if (event) {
      return (
        <span className="inline-flex items-center gap-2 rounded-full bg-black/10 dark:bg-white/10 px-3.5 py-1.5 text-base">
      ...
      <div className="container">
        ...
        <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="inline-flex rounded-2xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-white/5 p-1 backdrop-blur">
            {t.days.map((d) => (
              <button
                ...
                className={"relative rounded-xl px-5 py-2.5 text-base font-medium transition " + (activeDay===d ? "bg-accent text-white shadow-inner" : "text-black/70 dark:text-white/70 hover:text-black dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/10")}
              >
                {d}
              </button>
            ))}
          ...
          <div className="flex items-center gap-2">
            <label htmlFor="teamFilter" className="text-base muted">{t.filterLabel}</label>
            <input
              id="teamFilter"
              value={teamFilter}
              onChange={(e)=>setTeamFilter(e.target.value.slice(0,30))}
              placeholder={t.filterPh}
              className="w-52 rounded-xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-white/5 px-4 py-2.5 text-base placeholder-black/40 dark:placeholder-white/40 outline-none focus:ring-2 focus:ring-emerald-400/40"
            />
            {teamFilter && <button onClick={()=>setTeamFilter("")} className="rounded-lg px-3 py-2 text-sm text-black/70 dark:text-white/70">{t.clear}</button>}
          </div>
        </div>
    
        
        {/* Mobile list */}
        <ul className="md:hidden divide-y divide-black/10 dark:divide-white/10 rounded-2xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-white/5">
          {filteredRows.map((row, i) => (
            <li key={i} className="px-4 py-3">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="text-sm text-black/60 dark:text-white/60">{row.time}</div>
                  <div className="mt-1">
                    {row.event ? (
                      ...
                      (() => {
                        const labels = labelsFromMatch(row.match);
                        if (!labels) return <span className="font-medium">{row.match}</span>;
                        const { A, B } = labels;
                        return (
                          <div className="flex flex-wrap items-center gap-2 whitespace-normal">
                            <TeamBadge team={A} />
                ...
                <div className="shrink-0 pl-2 pt-4 text-base font-semibold">{row.event ? '—' : '0–0'}</div>
              </div>
            </li>
          ))}
        </ul>
        ...
        <div className="overflow-x-auto rounded-2xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-white/5 shadow-xl">
          <table className="hidden md:table w-full table-auto min-w-[980px]">
            <thead>
              <tr className="border-b border-black/10 dark:border-white/10 bg-white/50 dark:bg-white/5">
                <th className="px-4 py-3 text-left text-sm font-semibold uppercase tracking-wider text-black/80 dark:text-white/80 text-sm">#</th>
                <th className="px-4 py-3 text-left text-sm font-semibold uppercase tracking-wider text-black/80 dark:text-white/80 text-sm">{t.timeCol}</th>
                <th className="px-4 py-3 text-left text-sm font-semibold uppercase tracking-wider text-black/80 dark:text-white/80 text-sm">{t.matchCol}</th>
                <th className="px-4 py-3 text-left text-sm font-semibold uppercase tracking-wider text-black/80 dark:text-white/80 text-sm">{t.result}</th>
              </tr>
            </thead>
            <tbody>
              {filteredRows.map((row, i) => (
                <tr key={i} className="align-top hover:bg-black/5 dark:hover:bg-white/5">
                  <td className="px-4 py-3 text-base whitespace-nowrap text-black/80 dark:text-white/80">{i+1}</td>
                  <td className="px-4 py-3 font-medium whitespace-nowrap text-base">{row.time}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-base"><MatchCell match={row.match} event={row.event} /></td>
                  <td className="px-4 py-3 font-semibold whitespace-nowrap text-base">{row.event ? '—' : '0–0'}</td>
                </tr>
              ))}
            </tbody>
          </table>
