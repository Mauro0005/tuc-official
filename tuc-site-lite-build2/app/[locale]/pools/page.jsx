  ...
  return (
    <div className="h-full flex flex-col rounded-2xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-white/5 p-5 min-h-[360px]">
      ...
      <div className="mt-6 overflow-x-auto">
        <table className="w-full table-auto min-w-[720px] text-sm">
          <thead>
            <tr className="border-b border-black/10 dark:border-white/10 bg-white/50 dark:bg-white/5">
              <th className="w-[6%] px-2 py-2 text-left font-semibold text-black/70 dark:text-white/70">#</th>
              <th className="w-[48%] px-2 py-2 text-left font-semibold text-black/70 dark:text-white/70">Team</th>
              <th className="w-[9%] px-2 py-2 text-right font-semibold text-black/70 dark:text-white/70">W</th>
              <th className="w-[9%] px-2 py-2 text-right font-semibold text-black/70 dark:text-white/70">L</th>
              <th className="w-[9%] px-2 py-2 text-right font-semibold text-black/70 dark:text-white/70">PF</th>
              <th className="w-[9%] px-2 py-2 text-right font-semibold text-black/70 dark:text-white/70">PA</th>
              <th className="w-[10%] px-2 py-2 text-right font-semibold text-black/70 dark:text-white/70">+/-</th>
            </tr>
          </thead>
          <tbody>
            {teams.map((code, idx)=>(
              <tr key={code} className="hover:bg-black/5 dark:hover:bg-white/5">
                <td className="px-2 py-1.5 text-left font-mono">{idx+1}.</td>
                <td className="px-2 py-1.5">
                  <div className="flex items-center gap-2 whitespace-nowrap overflow-hidden">
                    <img src={`/teams/${TEAM_MAP[code].slug}.png`} alt={TEAM_MAP[code].name} className="h-6 w-6 rounded object-cover" />
                    <span className="truncate max-w-[260px]">{TEAM_MAP[code].name}</span>
                  </div>
                </td>
                <td className="px-2 py-1 text-right font-mono">0</td>
                <td className="px-2 py-1 text-right font-mono">0</td>
                <td className="px-2 py-1 text-right font-mono">0</td>
                <td className="px-2 py-1 text-right font-mono">0</td>
                <td className="px-2 py-1 text-right font-mono">0</td>
