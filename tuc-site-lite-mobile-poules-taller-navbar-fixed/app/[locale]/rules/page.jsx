...
export default function Page({ params }) {
  ...
  const copy = {
    nl: {
      ...
      sections: [
        ...
        { h: "Time-outs", items: [ "1 per team per helft (niet overdraagbaar).", "In overtime: 1 extra per team." ]},
        { h: "Wissels", items: [ "Alleen bij dode bal; eerst melden bij de jurytafel." ]},
        { h: "Scoren", items: [ "Binnen de arc = 2 punten; buiten de arc = 3 punten; vrije worp = 1 punt." ]},
        { h: "Fouten", items: [
    ...
    en: {
      ...
      sections: [
        ...
        { h: "Time-outs", items: [ "1 per team per half (not carried over).", "In overtime: 1 extra per team." ]},
        { h: "Substitutions", items: [ "Only on dead balls; report at the scorer's table first." ]},
        { h: "Scoring", items: [ "Inside the arc = 2 pts; beyond the arc = 3 pts; free throw = 1 pt." ]},
        { h: "Fouls", items: [
  ...
  return (
    <section className="container py-12 md:py-16">
      ...
      <p className="mt-2 text-black/80 dark:text-white/80">{t.intro}</p>
      <div className="mt-8 space-y-8">
        {t.sections.map((sec,idx)=>(
          <section key={idx} className="rounded-2xl border border-white/10 bg-white/5 p-5">
            ...
            <ul className="mt-3 space-y-2">{sec.items.map((it,i)=>(<li key={i} className="text-black/80 dark:text-white/80">{it}</li>))}</ul>
          </section>
        ))}
      </div>
    </section>
