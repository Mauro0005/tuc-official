  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="container flex min-h-[58vh] flex-col items-center justify-center py-14 md:py-20">
        <img src="/logo.png" alt="TUC" className="mb-4 h-60 w-auto opacity-80" />
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          ...
          <Link href={`/${locale}/signup`} className="rounded-2xl border border-black/15 dark:border-white/15 px-5 py-3 text-sm font-semibold hover:bg-black/5 dark:hover:bg-white/10">{t.register}</Link>
        </div>
      </div>
      <div className="container pb-16">
        <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-4">
          {t.links.map((item)=>(
            <Link key={item.href} href={item.href} className="rounded-2xl border border-black/10 dark:border-white/10 bg-white/5 px-6 py-5 text-center font-medium hover:bg-black/5 dark:hover:bg-white/10">{item.label}</Link>
          ))}
        </div>
      </div>
    </section>
