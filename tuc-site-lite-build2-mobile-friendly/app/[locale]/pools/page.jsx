      ...
      <li key={code} className="px-4 py-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2">
            <span className="w-6 text-left font-mono">{idx+1}.</span>
            <img src={`/teams/${TEAM_MAP[code].slug}.png`} alt={TEAM_MAP[code].name} className="h-7 w-7 rounded object-cover" />
            <span className="truncate font-medium">{TEAM_MAP[code].name}</span>
          </div>
          <div className="shrink-0 text-right text-sm leading-5">
