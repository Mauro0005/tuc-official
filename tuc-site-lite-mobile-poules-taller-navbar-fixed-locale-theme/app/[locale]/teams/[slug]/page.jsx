...
function roster(){ return Array.from({length:10}).map((_,i)=>`John Doe ${i+1}`); }
export default function Page({ params }){
  const { slug, locale } = params;
  const team=TEAMS[slug]; if(!team) return <div className="container py-12">Team not found</div>;
  return (
    <section className="container py-12 md:py-16">
      <div className="flex items-center gap-4">
        <img src={`/teams/${slug}.png`} alt={team.name} className="h-16 w-16 rounded object-cover" />
        <div><h1 className="text-3xl md:text-4xl font-bold tracking-tight">{team.name}</h1>
          <p className="text-black/60 dark:text-white/60">{locale==='nl'?'Poule':'Pool'} {team.code}</p></div>
      </div>
