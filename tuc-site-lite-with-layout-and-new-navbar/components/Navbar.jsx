...
/** Theme toggle with Light / Dark / System (default = System). */
...
function MobileMenu({ open, onClose, links, locale }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] md:hidden">
      ...
      />
      {/* Sheet */}
      <div className="absolute inset-x-0 bottom-0 rounded-t-3xl border-t border-black/10 dark:border-white/10 bg-white dark:bg-neutral-900 shadow-2xl pb-[env(safe-area-inset-bottom)]">
        <div className="mx-auto w-full max-w-screen-sm p-5 pt-4">
          <div className="mb-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <img src="/logo.png" alt="TUC" className="h-6 w-auto" />
              <span className="font-semibold tracking-tight">{locale === "nl" ? "Menu" : "Menu"}</span>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="mt-2 text-center text-xs text-black/60 dark:text-white/60">
            {locale === "nl" ? "Sleep omlaag of tik buiten het venster om te sluiten" : "Swipe down or tap outside to close"}
          </div>
        </div>
      </div>
    </div>
...
export default function Navbar({ locale = "nl" }) {
  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-black/10 dark:border-white/10 bg-white/75 dark:bg-black/70 backdrop-blur pt-[env(safe-area-inset-top)]">
      <div className="container flex items-center gap-2 py-2 min-h-[56px]">
      ...
      {/* Mobile bottom-sheet menu (overlays everything) */}
