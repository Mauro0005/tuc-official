...
function TeamBadge({ code, team }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-lg border border-black/10 dark:border-white/10 bg-white/60 dark:bg-white/5 px-2 py-1 whitespace-nowrap">
      ...
      <span className="ml-1 rounded bg-black/10 dark:bg-white/10 px-1 py-0.5 text-[10px] text-black/70 dark:text-white/70">{code}</span>
    </span>
  );
}
...
export default function FullSchedule({ locale='nl' }) {
  const t = useMemo(() => ({
    ...
    note: locale==='nl'
      ? 'De tijden zijn indicatief; wedstrijden volgen elkaar direct op.'
      : 'Times are indicative; games run back-to-back.',
      ...
      <div className="container">
        ...
        <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="inline-flex rounded-2xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-white/5 p-1 backdrop-blur">
            {t.days.map((d) => (
              <button key={d} onClick={() => setActiveDay(d)} className={"relative rounded-xl px-4 py-2 text-sm font-medium transition " + (activeDay===d?"bg-accent text-white shadow-inner":"text-black/70 dark:text-white/70 hover:text-black dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/10")}>
                {d}
              </button>
            ))}
          </div>
          <div className="flex items-center gap-2">
            ...
            <input id="teamFilter" value={teamFilter} onChange={(e)=>setTeamFilter(e.target.value.slice(0,30))} placeholder={t.filterPh} className="w-44 rounded-xl border border-black/10 dark:border-white/10 bg-white/60 dark:bg-white/5 px-3 py-2 text-sm placeholder-black/40 dark:placeholder-white/40 outline-none focus:ring-2 focus:ring-emerald-400/40" />
            {teamFilter && <button onClick={()=>setTeamFilter('')} className="rounded-lg px-2 py-2 text-xs text-black/70 dark:text-white/70">{t.clear}</button>}
          </div>
        </div>
          ...
          <table className="hidden w-full table-auto md:table min-w-[980px]">
            <thead>
              <tr className="border-b border-black/10 dark:border-white/10 bg-white/50 dark:bg-white/5">
                <th className="px-4 py-3 whitespace-nowrap text-left text-xs font-semibold uppercase tracking-wider text-black/60 dark:text-white/60">#</th>
                <th className="px-4 py-3 whitespace-nowrap text-left text-xs font-semibold uppercase tracking-wider text-black/60 dark:text-white/60">{t.timeCol}</th>
                <th className="px-4 py-3 whitespace-nowrap text-left text-xs font-semibold uppercase tracking-wider text-black/60 dark:text-white/60">{t.matchCol}</th>
                <th className="px-4 py-3 whitespace-nowrap text-left text-xs font-semibold uppercase tracking-wider text-black/60 dark:text-white/60">{t.result}</th>
              </tr>
            </thead>
            <tbody>
              {filteredRows.map((row, i) => (
                <tr key={i} className="align-top hover:bg-black/5 dark:hover:bg-white/5">
                  <td className="px-4 py-3 text-sm text-black/80 dark:text-white/80">{i+1}</td>
                  <td className="px-4 py-3 font-medium">{row.time}</td>
                  <td className="px-4 py-3"><MatchCell match={row.match} event={row.event} /></td>
                  <td className="px-4 py-3 font-semibold">{row.event ? '—' : '0–0'}</td>
                </tr>
              ))}
            </tbody>
          </table>
